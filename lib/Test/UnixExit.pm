...
Existing unit tests (for example via L<Test::More>) can be converted
quite simply to gain the benefit of checking the additional status word
fields, as the exit code can be zero both when the process exits
normally and if it eats a fatal signal (shell code instead mangles these
two distinct states together into one value).
...
Untested with other test modules (e.g. L<Test>) besides L<Test::Most>.
